...
export function TelegramTemplatesPanel() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.telegram.listTemplates();
      setTemplates(data);
      setForms(
        Object.fromEntries(data.map((item) => [item.key, { textBody: item.textBody }])),
      );
  ...
  return (
    <Card className="border-0 shadow-card">
      <CardHeader>
        ...
        <CardDescription>
          Тексты уведомлений в Telegram. Переменные в фигурных скобках, например{" "}
          <code className="rounded bg-muted px-1">{"{{stageLabel}}"}</code>. Можно HTML:{" "}
          <code className="rounded bg-muted px-1">&lt;b&gt;</code>,{" "}
